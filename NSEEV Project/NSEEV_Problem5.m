...
number_of_runs = 100;
fixations = 100;
tolerance = 10^-3; %3 digit accuracy is desired
step = .0001;
median_goal = 3.89;
...
while (abs(prctile(alert_time,50)-median_goal) > tolerance)
    ...
    %Apply step to decrease the values
    Ef_ad = Ef_ad - step;
    Ef_bd = Ef_bd - step;
    Ef_cd = Ef_cd - step;
    %check for divergence
    if Ef_ad < 0 || Ef_bd < 0 || Ef_cd <0
        fprintf('*****Value did not converge, exiting loop**** \n')
        break;
    end 
    %recalculate NSEEV Probability values
    Pabs_atod = S_d - Ef_ad + Ex_d + V_d;
    Pabs_btod = S_d - Ef_bd + Ex_d + V_d;
    Pabs_ctod = S_d - Ef_cd + Ex_d + V_d;
    ...
    Prel_ctod = (Pabs_ctod) / (Pabs_ctoa + Pabs_ctob + Pabs_ctod);
    %error val
    fprintf('median: %4.3f goal median: %4.3f \n', prctile(alert_time,50),median_goal)
end 

%% Calculate result effort value
fprintf('new Ef_ad value: %4.2f \n', Ef_ad)
fprintf('new Ef_bd value: %4.2f \n', Ef_bd)
fprintf('new Ef_cd value: %4.2f \n', Ef_cd)
toc
